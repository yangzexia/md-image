...
PROGRAMS_TOBUILD=(llvm-ventus libclc ocl-icd driver pocl)
...
export_elements() {
  ...
  export SPIKE_SRC_DIR=${DIR}/../ventus-gpgpu-isa-simulator
  export SPIKE_TARGET_DIR=${VENTUS_INSTALL_PREFIX}/lib
}
...
check_if_spike_built() {
  if [ ! -f "${VENTUS_INSTALL_PREFIX}/lib/libspike_main.so" ];then
    if [ ! -f "${DIR}/../ventus-gpgpu-isa-simulator/build/libspike_main.so" ];then
      echo "Please build isa-simulator first!"
      exit 1
    else
      cp ${DIR}/../ventus-gpgpu-isa-simulator/build/libspike_main.so ${VENTUS_INSTALL_PREFIX}/lib/
    fi
  fi
}
